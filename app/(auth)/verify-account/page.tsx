...
const WHATSAPP_NUMBER = "2290159289208";
const WHATSAPP_DELAY  = 120; // secondes avant d'afficher le lien
...
function VerifyAccountForm() {
  ...
  function buildWhatsappUrl(userEmail: string) {
    const msg = encodeURIComponent(
      `Bonjour, je n'arrive pas à recevoir mon code de vérification WhatsPAY. Mon adresse email est : ${userEmail}`
    );
    ...
    >
      <div className="w-full max-w-sm rounded-xl bg-white px-7 py-8 shadow-[0_0_37px_rgba(8,21,66,0.05)]">
        ...
        <div className="mt-5 text-center space-y-3">
          ...
          {/* Lien WhatsApp — apparaît après 2 minutes */}
          {showWhatsapp ? (
            <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-gray-700">
              <p className="mb-2">Toujours pas reçu votre code ?</p>
              <a
                href={buildWhatsappUrl(email)}
                target="_blank"
            ...
            !resendMsg && (
              <p className="text-xs text-gray-400">
                Lien d&apos;assistance disponible dans{" "}
